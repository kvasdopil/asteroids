...
function init(scn) {
  flareTexture = new BABYLON.Texture("textures/flare.png", scn);
  SCN = scn;
}

function createExhaust(target) {
  const exhaust = new BABYLON.ParticleSystem("exhaust", 2000, SCN);
  exhaust.particleTexture = flareTexture;
...
function createOidExplosion(vector, target) {
  const explosion = new BABYLON.ParticleSystem("oid explosion", 2000, SCN);
  explosion.particleTexture = flareTexture;
...
function createBallExplosion(target) {
  const explosion = new BABYLON.ParticleSystem("ball explosion", 2000, SCN);
  explosion.particleTexture = flareTexture;
...
function createExplosion(target) {
  const explosion = new BABYLON.ParticleSystem("explosion", 2000, SCN);
  explosion.particleTexture = flareTexture;
...
function createFire(target) {
  const fire = new BABYLON.ParticleSystem("exhaust", 2000, scene);
  fire.particleTexture = flareTexture;
